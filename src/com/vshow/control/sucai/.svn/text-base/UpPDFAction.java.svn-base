...
public class UpPDFAction implements Action {
		...
		if(flag == 1){
			...
			if (new File(tpdir).mkdir()) {
				...
				if(Constant.OFFICETYPE==0){
					...
					+ "pdim.exe  pdf " + file.getAbsolutePath() + " " + tpdir;
					
				}else if(Constant.OFFICETYPE==1){
					cmdd = "cmd /c " + Constant.EXE + File.separator
					+ "pdim2.exe  pdf " + file.getAbsolutePath() + " " + tpdir+" "+Constant.OFFICEZOOM;
					
				}else if(Constant.OFFICETYPE==2){
					
					OfficeUtil.pdf2Img(file.getAbsolutePath(),tpdir);
					
					//OfficeUtil.pdfImgByIce(file.getAbsolutePath(),tpdir);
