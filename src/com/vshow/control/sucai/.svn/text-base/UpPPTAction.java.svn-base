...
public class UpPPTAction implements Action {
		...
		if(flag == 1){
			...
			if (new File(tpdir).mkdir()) {
				...
				if(Constant.OFFICETYPE==0){
					cmdd = "cmd /c " + Constant.EXE + File.separator+ "pdim.exe  ppt " + file.getAbsolutePath() + " " + tpdir;
					
				}else if(Constant.OFFICETYPE==1){
					cmdd = "cmd /c " + Constant.EXE + File.separator
					+ "pdim2.exe  ppt " + file.getAbsolutePath() + " " + tpdir+" "+Constant.OFFICEZOOM;;
				}else if(Constant.OFFICETYPE==2){
					
					String pdfFile=tpdir+".pdf";
					OfficeUtil.ppt2Img(file.getAbsolutePath(), pdfFile, tpdir+".png",tpdir);
				}else if(Constant.OFFICETYPE==3){
					String newFile=null;
					String pdfFile=tpdir+".pdf";
					String files=file.getAbsolutePath();
					if(fileFileName.endsWith(".ppt"))
					{
						newFile=files.substring(0, files.lastIndexOf("."))+".ppt";
					}else if(fileFileName.endsWith(".pptx"))
					{
						newFile=files.substring(0, files.lastIndexOf("."))+".pptx";
					}
					
					file.renameTo(new File(newFile));
					OpenOfficeUtil.ppt2Img(newFile, pdfFile, tpdir);
				}else if(Constant.OFFICETYPE==4)
				{
					...
					file.renameTo(new File(newFile));
					AsposeUtil.ppt2Img(newFile, pdfFile, tpdir);
				}
