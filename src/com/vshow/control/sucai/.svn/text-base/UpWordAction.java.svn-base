...
public class UpWordAction implements Action {
		...
		if(flag == 1){
			...
			if (new File(tpdir).mkdir()) {
				...
				if(Constant.OFFICETYPE==0){
					...
					if(fileFileName.endsWith(".doc"))
					{
						 newFile=files.substring(0, files.lastIndexOf("."))+".doc";
						 
					}else if(fileFileName.endsWith(".docx"))
					{
						 newFile=files.substring(0, files.lastIndexOf("."))+".docx";
					}
					
					file.renameTo(new File(newFile));
					OpenOfficeUtil.word2Img(newFile, pdfFile, tpdir);
					...
					file.renameTo(new File(newFile));
					AsposeUtil.word2Img(newFile, pdfFile, tpdir);
				}
