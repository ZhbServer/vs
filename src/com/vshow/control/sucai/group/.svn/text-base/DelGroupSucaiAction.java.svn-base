...
public class DelGroupSucaiAction extends ActionSupport {
	...
	private int type;//分组类型（素材类型 1 背景 2音频 3视频 4图片 5FLASH 6PPT 7Word 8Excel 9PDF 10文本）
	...
	public String execute() throws Exception {
		...
		for (int i = 0; i < qx.length; i++) {
			...
			String str = (String) SqlConnect.getSqlMapInstance()
					.queryForObject("sel_scgroup_name_id", qx[i]);
            //查询这个分组下的所有素材id
			List<Integer> gsscids=SqlConnect.getSqlMapInstance().queryForList("sel_scgroup_sc_id_all",qx[i]);
			...
			SqlConnect.getSqlMapInstance().delete("del_scgroup_id", qx[i]);
		}
		...
		if(sclogcontent==""){
			LogHandle.insertTheLog(LogType.type5, urid,c1+":" + logcontent,1,1);
		}else{
			LogHandle.insertTheLog(LogType.type5, urid, c1+":" + logcontent+"("+c2+":"+sclogcontent+")",1,1);
		}
