...
public class ToSeeGroupSucaiAction implements Action {
	...
	private List<ScGroup> scgs; 
  
    private int type; //分组类型（素材类型 1 背景）
	...
	public String execute() throws Exception {
		...
		total = (Integer) SqlConnect.getSqlMapInstance().queryForObject("sel_scgroup_page_count", tempsg);
		...
		scgs=SqlConnect.getSqlMapInstance().queryForList("sel_scgroup_page", tempsg);
		subStringListGroup(scgs);
	...
	// 显示截取名字(素材)
	public void subStringListGroup(List<ScGroup> list) {
		for (ScGroup gs : list) {
			if(gs!=null){
	...
	public void setScgs(List<ScGroup> scgs) {
		this.scgs = scgs;
	}
