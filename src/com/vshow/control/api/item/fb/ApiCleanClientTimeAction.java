 ...
 *
 */
public class ApiCleanClientTimeAction extends ActionSupport{
		...
		if (user == null) {
			...
			if (user.getBan() == 0) {
				...
				try{	
					...
					if(tmidsset.size()>0){
						String tname = Constant.LOCAL.getA00590()+"["+Constant.LOCAL.getA00533()+"]";
						Log log = LogHandle.insertTheLogSend(LogType.type37, user.getId(),tname,1);
