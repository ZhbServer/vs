 ...
 *
 */
public class ApiFbTimeJhItemAction extends ActionSupport{
				...
				try{	
					...
					if(tmidsset.size()>0){
						...
						for (JhItem jhitem : jhitems) {
							playcon = Constant.LOCAL.getA00151()+":" + jhitem.getItemnames();
							if(jhitem.getDay()==8){
								dayString=Constant.LOCAL.getA00483();
							}else if(jhitem.getDay()==1){
								dayString=Constant.LOCAL.getA00475();
							}else if(jhitem.getDay()==2){
								dayString=Constant.LOCAL.getA00476();
							}else if(jhitem.getDay()==3){
								dayString=Constant.LOCAL.getA00478();
							}else if(jhitem.getDay()==4){
								dayString=Constant.LOCAL.getA00479();
							}else if(jhitem.getDay()==5){
								dayString=Constant.LOCAL.getA00480();
							}else if(jhitem.getDay()==6){
								dayString=Constant.LOCAL.getA00481();
							}else if(jhitem.getDay()==7){
								dayString=Constant.LOCAL.getA00482();
							}
							
							playcon += Constant.LOCAL.getA00493()+" :" +dayString ;
							...
							if(cmds.length()==0){
								cmds=jhitem.getDay()+"_"+jhitem.getSdate()+"_"+jhitem.getEdate()+"_"+cmd;
							}else{
								cmds=cmds+"^"+jhitem.getDay()+"_"+jhitem.getSdate()+"_"+jhitem.getEdate()+"_"+cmd;
							}
						...
						if (itemset.size() > 0) {
							...
							String logcontent =Constant.LOCAL.getA00653()+"["+Constant.LOCAL.getA00647()+"]:"+jhName;
							Log log = LogHandle.insertTheLogSend(LogType.type89, user.getId(), logcontent,1);
							String sendkey = log.getSendkey();
							String msg = "02" + clen + "/" + downcmd + "+"
									+ "sendkey!" + sendkey + "%timelist!" + cmds;		
							log.setMsg(msg);
							...
							for (Integer cid : tmidsset) {
								...
								for (JhItem jhitem : jhitems) {
									...
									SqlConnect.getSqlMapInstance().insert("insert_one_timeitem", st);
									
								}
								
								/**添加执行结果最后一次播放记录**/
								//itemsids=itemsids.replaceAll("_", ",");
								ClientPlayHandle.insertClientPlay(itemidss, 1, ResultType.rt16, playcons, user.getId(), client.getMark(), log.getSendkey(), 0);
								code = "2";
