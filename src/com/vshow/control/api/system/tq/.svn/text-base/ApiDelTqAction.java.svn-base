 ...
 * 
 */
public class ApiDelTqAction extends ActionSupport {
			...
			if (user.getBan() == 0) {
				...
				try {
					...
					for (int i = 0; i < qid.length; i++) {
						tq=(Tq)SqlConnect.getSqlMapInstance().queryForObject("g_wotq_value",Integer.parseInt(qid[i]));
						SqlConnect.getSqlMapInstance().delete("del_tq", Integer.parseInt(qid[i]));
						LogHandle.insertTheLog(LogType.type62, user.getId(),Constant.LOCAL.getA00185()+":" + tq.getCity(), 1,1);
					}
					code = "2";
				} catch (Exception e) {
