...
public class AddTreasureListAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		creatorTime=df.format(new Date());// new Date()为获取当前系统时间
						
		String code=uploadCodeFile();//二维码
				
		String img=uploadImgFile();//图片		
		
		int imgHeights=0;
		for (int i = 0; i < imgHeight.length; i++) {
			
			if (imgHeight[i]!=null&&!imgHeight[i].equals("")) {//判断图片高度是否为空，不为空则赋值给imgHeights
		...
		for (int j = 0; j < imgWidth.length; j++) {
			...
			if (imgWidth[j]!=null&&!imgWidth[j].equals("")) {
		...
		String styleImg=uploadStyleImgFile();//宝箱样式图片	
		...
		Tre.setSkipImg(img);	
		Tre.setCreator(creator);//创建人	
		Tre.setCreatorTime(creatorTime);
		Tre.setLastModifier(null);//修改人，默认为空
		Tre.setModifierTime(null);//修改时间	
		Tre.setBan(ban);
		...
		int num=(Integer) SqlConnect.getSqlMapInstance().insert("add_treasure", Tre);
		if (num>0) {
	...
	public String uploadCodeFile() {
		if (codeNameFileName!=null) {
			...
			String tempf=null;
			if (styleImgNameFileName!=null&&!styleImgNameFileName.equals("")) {
