 ...
 *
 */
public class SetDevicePlayStopAction extends BaseAction {

	
	private Integer areaCode;  //区域表示/分组id 0表示默认
	private String  deviceMark;//终端标识
	private int  type; //1播放 3停止  ，2暂停 4继续
				 ...
				 if(client==null){
					 ...
					 if(client.getZxstate()==1){
						 ...
						 if (mt != null) {
				        	 ...
				        	 protocol.setType(ProtocolType.TYPE73);
				        	 
				        	 ProtocolPlayStop playStop=new ProtocolPlayStop();
				        	 
				        	 playStop.setPlayStop(type);
				        	 
				        	 List<ProtocolPlayStop> playStopList=new ArrayList<ProtocolPlayStop>();
				        	 
				        	 playStopList.add(playStop);
				        	 
				        	 protocol.setProtocolPlayBaseList(playStopList);
