...
import com.vshow.control.protocol.ProtocolPlayImgNext;
 ...
 *
 */
public class SetDevicePlayImgNextAction extends BaseAction {

	
	private Integer areaCode;  //区域表示/分组id 0表示默认
	private String  deviceMark;//终端标识
	private int  type;  // 1上一张  2下一张  
				 ...
				 if(client==null){
					 ...
					 if(client.getZxstate()==1){
						 ...
						 if (mt != null) {
				        	 ...
				        	 protocol.setType(ProtocolType.TYPE75);
				        	 
				        	 ProtocolPlayImgNext playImgNext=new ProtocolPlayImgNext();
				        	 
				        	 playImgNext.setPlayImgNext(type);
				        	 
				        	 List<ProtocolPlayImgNext> playImgNextList=new ArrayList<ProtocolPlayImgNext>();
				        	 
				        	 playImgNextList.add(playImgNext);
				        	 
				        	 protocol.setProtocolPlayBaseList(playImgNextList);
							 ...
							 System.out.println("请求发送放入队列时间:"+DateUtil.getTime());
