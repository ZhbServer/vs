 ...
 *
 */
public class ApiTimeWriteInfoAction extends ActionSupport{
			...
			if (user.getBan() == 0) {
				...
				try{	
					...
					if(tmidsset.size()>0){
						String ctstring="";
						if(cameratype==1){
							ctstring=Constant.LOCAL.getA00696();
						}else if(cameratype==2){
							ctstring=Constant.LOCAL.getA00697();
						}
						String tname=ctstring+"^"+Constant.LOCAL.getA00698()+"^"+sdate+"^"+edate;
						//String tname = local.getA00468()+":" + vol;
						Log log = LogHandle.insertTheLogSend(LogType.type81, user.getId(),tname,1);
						String sendkey = log.getSendkey();
						String msg = "36+"+cameratype+"^"+sdate+"^"+edate+"%sendkey!" + sendkey ;
						log.setMsg(msg);
