 ...
 * @param account账号
 * @param password 密码 
 * @param clientpassword 终端密码
 * @param cids 终端id
 * @author cyn
 * @version 6.1 API接口
 *
 */
public class ApiClientPwdAction extends ActionSupport{
		...
		if (user == null) {
			...
			if (user.getBan() == 0) {
				...
				try{	
					...
					if(tmidsset.size()>0){
						Log log = LogHandle.insertTheLogSend(LogType.type95, user.getId(),Constant.LOCAL.getA00769(),1);
						String sendkey = log.getSendkey();
						String msg = "50+sendkey!" + sendkey + "%" + clientpassword;
						log.setMsg(msg);
						...
						for (Integer cid : tmidsset) {
							...
							if(cpaw==null){
								...
								clientpaw.setCpassword(clientpassword);
