 ...
 *
 */
public class ApiVolSetAction extends ActionSupport{
			...
			if (user.getBan() == 0) {
				...
				try{	
					...
					if(tmidsset.size()>0){
						...
						String tname = Constant.LOCAL.getA00468()+":" + vol;
						Log log = LogHandle.insertTheLogSend(LogType.type29, user.getId(),tname,1);
						String sendkey = log.getSendkey();
						String msg = "08+sendkey!" + sendkey + "%" + ts;
						log.setMsg(msg);
