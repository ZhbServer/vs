 ...
 *
 */
public class ApiRestartAction extends ActionSupport{
		...
		if (user == null) {
			...
			if (user.getBan() == 0) {
				...
				try{	
					...
					if(tmidsset.size()>0){
						String tname = Constant.LOCAL.getA00486();
						Log log = LogHandle.insertTheLogSend(LogType.type24, user.getId(),tname,1);
						String sendkey = log.getSendkey();
						String msg = "12+"+ sendkey ;
						log.setMsg(msg);
