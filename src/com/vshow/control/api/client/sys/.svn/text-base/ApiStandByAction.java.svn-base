 ...
 *
 */
public class ApiStandByAction extends ActionSupport{
		...
		if (user == null) {
			...
			if (user.getBan() == 0) {
				...
				try{	
					...
					if(tmidsset.size()>0){
						String tname = Constant.LOCAL.getA00488();
						Log log = LogHandle.insertTheLogSend(LogType.type26, user.getId(),tname,1);
						String sendkey = log.getSendkey();
						String msg = "06+"+ sendkey ;
						log.setMsg(msg);
