 ...
 *
 */
public class ApiTouchSpaceTimeAction extends ActionSupport{
		...
		if (user == null) {
			...
			if (user.getBan() == 0) {
				...
				try{	
					...
					if(tmidsset.size()>0){
						String key = "01+";
						String ts = "urltime!" + urltime;
						String tname = Constant.LOCAL.getA00474()+":" + urltime;

						
						Log log = LogHandle.insertTheLogSend(LogType.type32, user.getId(),tname,1);
