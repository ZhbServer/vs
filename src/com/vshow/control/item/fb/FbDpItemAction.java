...
public class FbDpItemAction implements Action {
	...
	private Integer jgtime;     //间隔时间(分钟) 
		...
		if(tmidsset.size()>0){
			...
			if (itemids_s.length > 0) {
				...
				for (int i = 0; i < itemids_s.length; i++) {
				    ...
				    String logcontent = local.getA00150()+":" + itemnames;
				    //String temstr;
				
				
				    // 垫片
					DpSend ds = new DpSend();

					// 开始时间
					ds.setSdate(sdate.trim());
					logcontent += local.getA00582() + " :" + ds.getSdate();

					// 结束时间
					ds.setEdate(edate.trim());
					logcontent += local.getA00583() + " :" + ds.getEdate();

					// 间隔时间
					ds.setIdate(jgtime);
					logcontent += local.getA00584() + " :" + jgtime;
					// 播放次数
					ds.setPcount(playcount);
					logcontent += local.getA00585() + " :" + playcount;
					
					/** 判断用户有没有发送权限(0.需审核 1.已审核.2节目需审核3.发布需审核)**/
					if (uocheck == 0||uocheck==3) {
						
						Log log = LogHandle.insertTheLogSend(LogType.type73, uid, logcontent,2);
						...
						for (Integer cid : tmidsset) {
							...
							ds.setResultid(rid);
							ds.setItemid(itemsids);
							sm.insert("insert_one_dpitem", ds);
						}
						...
						for (int i = 0; i < scenes.size(); i++) {
							...
							if (i == 0) {
								...
								cmd = ds.getSdate() + "_" + ds.getEdate() + "_"
										+ ds.getIdate() + "_" + ds.getPcount() + "_"
										+ sce.getFilename();
						...
						String msg = "02" + clen + "/" + downcmd + "+" + "sendkey!" + sendkey + "%dplist!" + cmd;	
						log.setMsg(msg);
						...
						for (Integer cid : tmidsset) {
							...
							ClientPlayHandle.insertClientPlay(itemsids, 1, ResultType.rt18, logcontent, uid, client.getMark(), log.getSendkey(), 0);
