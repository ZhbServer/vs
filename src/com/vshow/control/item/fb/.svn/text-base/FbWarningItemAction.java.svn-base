...
public class FbWarningItemAction implements Action {
	...
	public String execute() throws Exception {
		...
		String uAccount=(String)session.get("account");
		...
		if(tmidsset.size()>0){
			...
			if (itemArray.length > 0) {
				...
				for (int i = 0; i < itemArray.length; i++) {
					//itemid=Integer.parseInt(itemArray[i]);
					itemid=Integer.parseInt(itemArray[i].split("\\,")[0]);                          
					playNumber=1;
					/**获取节目信息**/
					...
					if (uocheck == 0||uocheck==3) {
					    ...
					    String logcontent =local.getB00275()+":" + itemnames;
						
					    
					    Log log = LogHandle.insertTheLogSend(LogType.type108, uid, logcontent,1);
