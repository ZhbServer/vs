...
public class FbInsertItemAction implements Action {
		   ...
		   if(VVersion.le_clientdisabled==0){
			...
			if (itemids_s.length > 0) {
				...
				for (int i = 0; i < itemids_s.length; i++) {
				    ...
				    String logcontent = local.getA00152()+":" + itemnames;
				    //String temstr;
				
				
				    // 插播
					InsertSend isd = new InsertSend();

					isd.setXiadate(xiadate.trim());
					logcontent += local.getA00581()+" :" + isd.getXiadate();

					isd.setSdate(sdate.trim());
					logcontent += local.getA00582()+" :" + isd.getSdate();

					isd.setEdate(edate.trim());
					logcontent += local.getA00583()+" :" + isd.getEdate();
					
					/** 判断用户有没有发送权限(0.需审核 1.已审核.2节目需审核3.发布需审核)**/
					if (uocheck == 0||uocheck==3) {
						
						Log log = LogHandle.insertTheLogSend(LogType.type14, uid, logcontent,2);
						...
						for (Integer cid : tmidsset) {
							...
							isd.setResultid(rid);
							isd.setItemid(itemsids);
							sm.insert("insert_one_insertitem", isd);
						}
						...
						for (int i = 0; i < scenes.size(); i++) {
							...
							if (i == 0) {
								...
								cmd = isd.getSdate() + "_" + isd.getEdate() + "_"
										+ sce.getFilename();
						...
						Log log = LogHandle.insertTheLogSend(LogType.type14, uid, logcontent,1);
						String sendkey = log.getSendkey();
						String msg = "02" + clen + "/" + downcmd + "+"+ "sendkey!" + sendkey + "%cblist!" + cmd;	
						log.setMsg(msg);
						...
						for (Integer cid : tmidsset) {
							...
							ClientPlayHandle.insertClientPlay(itemsids, 1, ResultType.rt17, logcontent, uid, client.getMark(), log.getSendkey(), 0);

							if (Constant.compare_date(fmt.format(time), xiadate)==0||Constant.compare_date(fmt.format(time), xiadate)==1)

							{
							
								//更新状态
								sm.update("update_insertsend_state", rid);
