...
public class FbInductionInteractItemAction implements Action {
	...
	public String execute() throws Exception {
		...
		if(tmidsset.size()>0){
			...
			InteractItem item = (InteractItem) sm.queryForObject(
					"sel_id_interactitem", Integer.parseInt(itemsids));
			Client client;
			Scene se;
			String filepathzip="";
			int fileExists=0;
			...
			if (tmidsset.size() > 0) {
				...
				String filenNameR =(String)sm.queryForObject("sel_interact_item_filename_id", Integer.parseInt(itemsids));
				...
				if (uocheck == 0 || uocheck == 3) {
					...
					String logcontent =local.getB00089()+":" + item.getName();
				
					Log log = LogHandle.insertTheLogSend(LogType.type98, uid, logcontent,1);
					String sendkey = log.getSendkey();
					String msg = "02" + clen + "/" + downcmd + "+"
							+ "sendkey!" + sendkey + "%inductionlist!" + cmd;
					log.setMsg(msg);
