...
public class SaveSubSceneAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		if(itemType==1){
		    ...
		    obj = sm.queryForObject("sel_scfilename_scene_sub", mfile);
			if (obj != null) {
			...
			// 先查询节目是否存在
			// 获取节目
			interactItem = (InteractItem) session.get("interactitem");
			if (interactItem.getId() == 0) {
				// 插入节目
				
				Integer iid=(Integer)sm.insert("insert_one_interactitem", interactItem);
				// 并且返回刚插入节目的信息
				interactItem = (InteractItem) sm.queryForObject("sel_id_interactitem", iid);
				
				session.put("interactitem", interactItem);
				LogHandle.insertTheLog(LogType.type6, urid, Constant.LOCAL.getA00443()+ ":" + interactItem.getName(), 1,1);
			    ...
			    iisg.setItemid(interactItem.getId());
			    iisg.setItemgroupid(igid);
				sm.insert("add_interactitem_group_relation", iisg);
				
			} else {
				// 节目已存在查找
				interactItem = (InteractItem) sm.queryForObject("sel_id_interactitem", interactItem.getId());
				
				int ocheck = (Integer) session.get("urcheck");
				if (ocheck == 0 || ocheck == 2) {
					interactItem.setOcheck(0);
				} else if (ocheck == 1 || ocheck == 3) {
					interactItem.setOcheck(1);
				}
				
				if(interactItem.getUserid()!=urid){
					//修改共享节目
					LogHandle.insertTheLog(LogType.type8, urid, local.getA00833()+"["+local.getA00729()+"]["+local.getB00015()+"]"
							+ ":" + interactItem.getName(), 1,1);
    ...
    //*************************************协议生成****************************************************
			...
			String x86tev = "";
			MorphDynaBean md;
			//分辨率(Scene) session.get("scene");
			Scene tempScene = (Scene) session.get("subscene");
	    ...
	    //*********************************协议生成结束*************************************************
	     ...
	     f = new File(dir + File.separator + "programSub.txt");
		...
		//查找场景内容存在不存在
	    SceneContent sContent=(SceneContent)sm.queryForObject("sel_scene_content_filename_sub", mfile);	
	    ...
	    if(sContent==null){
			...
			sContent.setType(itemType);
			sm.insert("insert_scene_content_sub", sContent);
	    }else{
	    	sContent.setContent(jsonArray2.toString());
			
			sm.update("update_scene_content_sub", sContent);
	    }
		...
		zip.zip(dir, dir + ".zip");
	     ...
	     // 获取场景(查找场景是否存在)
		 if (obj == null) {
				scene = (Scene) session.get("subscene");
				...
				scene.setSf(sf);
				scene.setFbltype(screenType);
				Integer sid=(Integer)sm.insert("insert_one_scene_sub", scene);
				...
				*/
				...
				scene.setFcontent(fcontent);
				scene.setSf(sf);
				sm.update("update_scene_sub", scene);				
			}
		
		   //遍历节目下所有场景,视频,音频文件字节大小
		   Scene fnVn=null;
		   if(itemType==1){
			   fnVn= (Scene)sm.queryForObject("sel_fnvn_iid", item.getId());
		   }else if(itemType==2){
			   fnVn= (Scene)sm.queryForObject("sel_fnvn_iid", interactItem.getId());
		   }   
		...
		if(itemType==1){
			 item.setLen(clen);
			 sm.update("upd_item2_sub", item);
		}else if(itemType==2){
			interactItem.setLen(clen);
			sm.update("upd_interactitem2_sub", interactItem);
		}
		...
		itemEditHis.setItemType(itemType);
