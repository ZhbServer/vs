...
public class ToInteractItemNoOcheckListAction implements Action {
	
	
	private List<InteractItem> listis; // 节目集合
	...
	public String execute() throws Exception {
		...
		if(sel==0){
			listis= SqlConnect.getSqlMapInstance().queryForList("sel_nocheckinteractitem", u);
			total = (Integer) SqlConnect.getSqlMapInstance().queryForObject("sel_nocheckinteractitem_count", u);
		}else if(sel==1){
			//item.setName(ser);
	...
	public List<InteractItem> getListis() {
		return listis;
	}
	...
	public void setListis(List<InteractItem> listis) {
		this.listis = listis;
	}
