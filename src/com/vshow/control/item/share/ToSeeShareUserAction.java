...
public class ToSeeShareUserAction implements Action {
	...
	public String execute() throws Exception {
		...
		int count = 0;
		Integer userid = (Integer)SqlConnect.getSqlMapInstance().queryForObject("get_item_userid_id",Integer.parseInt(itemid));
		if(userid != uid){
			count = 1;
		}
		if(count > 0){
			session.put("error", "#session.local.a00829");
			return ERROR;
		}
		...
		total = (Integer) SqlConnect.getSqlMapInstance().queryForObject(
				"sel_users_tkey_page_count", user);
