...
public class FbInsertJhItemAction implements Action {
			...
			if (itemset.size() > 0) {
				...
				if (uocheck == 0||uocheck==3) {
				      ...
				      **/
					    ...
					    String logcontent =local.getA00653()+"["+local.getA00648()+"]:"+jhName;
						Log log = LogHandle.insertTheLogSend(LogType.type90, uid, logcontent,1);
						...
						for (Integer cid : tmidsset) {
							...
							//Constant.addCml(cid, rid, log.getId(),log.getSendkey());
							//插入插播预警表
							//SqlConnect.getSqlMapInstance().delete("del_warning_item",client.getMark());
							
							ClientWarningItem clientWarningItem;
							int count =0;
							for (JhItem jhitem : jhitems) {
								...
								if(count==0){
									...
									sm.insert("insert_one_insertitem", is);
								...
								/*
								clientWarningItem=new ClientWarningItem();
								clientWarningItem.setItemfiles(jhitem.getItemids());
								clientWarningItem.setState(0);
								clientWarningItem.setType(17);
								clientWarningItem.setSdate(jhitem.getSdate());
								clientWarningItem.setEdate(jhitem.getEdate());
								clientWarningItem.setMark(client.getMark());
								clientWarningItem.setMsg("");
								clientWarningItem.setSendkey(log.getSendkey());
								SqlConnect.getSqlMapInstance().insert("ad_warning_item", clientWarningItem);
							    */
							...
							ClientPlayHandle.insertClientPlay(itemidss, 1, ResultType.rt17, playcons, uid, client.getMark(), log.getSendkey(), 0);
