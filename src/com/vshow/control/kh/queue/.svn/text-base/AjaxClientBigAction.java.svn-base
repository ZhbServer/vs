...
public class AjaxClientBigAction extends ActionSupport {
	private int type;
	private int cgid;
	...
	public String execute() throws Exception {
		...
		if(sel==0){
			clients = (List<Client>) SqlConnect.getSqlMapInstance().queryForList("sel_client_big_page", cgr);
			total=(Integer)SqlConnect.getSqlMapInstance().queryForObject("sel_client_big_page_count", cgr);
		}else{
			if(sel==1){
				cgr.setSname("name");
			}else if(sel==2){
			...
			cgr.setSer(ser.trim());
			clients = (List<Client>) SqlConnect.getSqlMapInstance().queryForList("sel_client_ser_big_page", cgr);
			total=(Integer)SqlConnect.getSqlMapInstance().queryForObject("sel_client_ser_big_page_count", cgr);
		}
