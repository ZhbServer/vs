...
public class ToCutListAjaxAction extends ActionSupport {
	private int type;
	private int cgid;
	...
	public String execute() throws Exception {
		...
		if(sel==0){
			clients = (List<Client>) SqlConnect.getSqlMapInstance().queryForList("sel_view_cut_client_sum", cgr);
			total=(Integer)SqlConnect.getSqlMapInstance().queryForObject("sel_view_cut_client_sum_count", cgr);
		}else{
			if(sel==1){
				cgr.setSname("name");
			}else if(sel==2){
			...
			cgr.setSer(ser.trim());
			clients = (List<Client>) SqlConnect.getSqlMapInstance().queryForList("sel_view_cut_client_sum_ser", cgr);
			total=(Integer)SqlConnect.getSqlMapInstance().queryForObject("sel_view_cut_client_sum_ser_count", cgr);
		}
		if(pagetype==0){
		}else{
			if (total % pagetype > 0) { 
