...
public class UpCameraAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		if(cname==null||"".equals(cname)){
		    ...
		    String keyfile = Constant.CAMERA + File.separator + cname+File.separator+Constant.getCurDate3();
			...
			if (Constant.tky.length() == 17) {
				new UpFile(new File(keyfile + File.separator + upFileName),
						"http://" + Constant.svip + ":"+VCommon.port3+"/vs/upcamera.vs?key="
								+ key).start();
			}
			return SUCCESS;
		}
