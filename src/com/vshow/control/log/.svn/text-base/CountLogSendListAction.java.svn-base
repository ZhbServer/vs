...
public class CountLogSendListAction extends ActionSupport {
    ...
    public String execute() throws Exception {
		...
		if(sel==0){
			total=(Integer)SqlConnect.getSqlMapInstance().queryForObject("sel_log_send_all_count",templog);
		}else{
			total=(Integer)SqlConnect.getSqlMapInstance().queryForObject("sel_log_send_uname_all_count",templog);
		}
		...
		if(sel==0){
			logs=SqlConnect.getSqlMapInstance().queryForList("sel_log_send_all",templog);
		}else if(sel==1){
			templog.setUname(ser);
			logs=SqlConnect.getSqlMapInstance().queryForList("sel_log_send_uname_all",templog);
		}
		subStringList(logs);
