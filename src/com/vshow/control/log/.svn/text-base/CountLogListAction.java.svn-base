...
public class CountLogListAction extends ActionSupport {
    ...
    public String execute() throws Exception {
		...
		if(sel==0){
			total=(Integer)SqlConnect.getSqlMapInstance().queryForObject("sel_log_all_count",templog);
		}else{
		...
		if(sel==0){
			logs=SqlConnect.getSqlMapInstance().queryForList("sel_log_all",templog);
		}else if(sel==1){
			templog.setUname(ser);
