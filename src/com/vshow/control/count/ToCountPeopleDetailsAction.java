...
public class ToCountPeopleDetailsAction implements Action {
	...
	public String execute() throws Exception {
		...
		{
			List<CountPeopleData> countlist=SqlConnect.getSqlMapInstance().queryForList(
					"sel_cp_list_info", cpd);
			countPeopleList=new ArrayList<CountPeopleData>();
			for (int i = 0; i < countlist.size(); i++) {
				JSONArray jsonArray=JSONArray.fromObject(countlist.get(i).getCountPeopleDetails());
				JSONObject jsonObject=JSONObject.fromObject(jsonArray.getJSONObject(0));
				String mac=jsonObject.get("mac").toString();
				if(mac==null)
				{
					countPeopleList.add(countlist.get(i));
				}
			}
		}else if(sel==2)//Wife
		{
