...
function del_sel(url){
    ...
    layer.confirm("You sure you want to delete it?", {icon: 3, title:"prompt"}, function(index){		 
...
function gogo(aurl) {
    ...
    //总页码
   	 ...
   	 if(parseInt(goid)>parseInt(totalpage)){
   	     layer.msg("Enter a page number is incorrect"); 
   	     return;
   	  }
...
function showcg1list(cgid,vurl,vtype){
	...
	var nextYearo = 'The default sort';  
...
function add_share(url,type){
     ...
     $.ajax({
 		...
 		success:function(data){
 	       if(data == "1"){
 	    	   layer.msg("Material that cannot be Shared to other users");
 	    	   return; 
 		   }else{
 			  layer.open({
 			  		type: 2,
...
function del_sucai(url){
    ...
    $.ajax({
		...
		success:function(data){
	       //if(data == "1"){
	       //	   layer.msg("Deletion can not be performed on a clip that contains a shared clip");
	       //	   return; 
		   //}else{
			  
			   layer.confirm("Confirm to delete?",{icon: 3, title:"Prompt",btn: ['Yes', 'No']}, function(index,layero){
