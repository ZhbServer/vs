...
function del_sel(url){
    ...
    layer.confirm("당신은 확실히 당신이 그것을 삭제할?", {icon: 3, title:"신속한"}, function(index){
...
function del_sucai(url){
    ...
    $.ajax({
		...
		success:function(data){
	       //if(data == "1"){
	       //   layer.msg("물질은 공유 삭제 동작이 수행 될 수 없다 포함");
	       //   return; 
		   //}else{
			 
			   
			   layer.confirm("삭제해야?",{icon: 3, title:"신속한",btn: ['Yes', 'No']}, function(index,layero){
				   document.form1.action=url;
				   document.form1.submit();
			   })
