...
function del_sucai(url){
    ...
    $.ajax({
		...
		success:function(data){
	       //if(data == "1"){
	       //	   layer.msg("包含共享的素材不能执行删除操作");
	       //	   return; 
		   //}else{
			   layer.confirm("确定删除吗?",{icon: 3, title:"提示",btn: ['确定', '返回']}, function(index,layero){
				   document.form1.action=url;
				   document.form1.submit();
			   });
		   //}
...
function checkPwd(pwdV) {
    ...
    if (complex < 4 || str.length < 6) {
        //layer.alert("包含大小写字母，数字,特殊字符不能包含%&\+,长度至少6位");
        layer.alert("包含大小写字母，数字,特殊字符不能包含%&\+,长度至少6位");
        return false;
    } else {
