...
function bindbus(){
     ...
     if(deviceId.length==0){
    	 layer.msg("<ww:property value='#session.local.a00773' />");
         return; 
     }
     //获取是否选中终端
     ...
     document.form1.action="adbindbus.vs?bindcid="+bindcid;
		...
		<div class="t_container">
			<div class="loc">
			       <ww:property value='#session.local.a00065' /> :
			        ...
			        <span class="material_bg"><ww:property value='#session.local.a00710' /></span>>
			        <span class="material_bg"><ww:property value='#session.local.a00772' /></span>
			</div>
...
<div class="mainframe clearfix user_list" id="mainCenter" style="color:red;height: 510px;">
            	<div class="mainCInLeft" style="height: 510px;width: 215px;">
							...
							try {
								...
								if (al.size() > 0) {
									...
									for (int j = 0; j < al.size(); j++) {
										...
										jw.print("<a  href='#' onclick='showcg1list("+cg2.getId()+",\"ajaxclientbus.vs\",2)' class='treeLink levlet_" + shendu
								...
								try {
									...
									if (al.size() > 0) {
										...
										for (int j = 0; j < al.size(); j++) {
											...
											jw.print("<a  href='#' onclick='showcg1list("+cg3.getId()+",\"ajaxclientbus.vs\",3)' class='treeLink levlet_" + shendu
					...
					<%
						...
						jw.print("<a href='#' onclick='showcg1list(0,\"ajaxclientbus.vs\",0)' class='treeLink levlet_1' style='height:40px;line-height:40px;font-size:12px;border-bottom:#e5e3da 1px solid;'  id='treeLink levlet_1'> ");
						...
						try {
							...
							if (al.size() > 0) {
								...
								for (int j = 0; j < al.size(); j++) { 
									...
									jw.print("<a  href='#' onclick='showcg1list("+cg1.getId()+",\"ajaxclientbus.vs\",1)' class='treeLink levlet_1' style='height:40px;line-height:40px;font-size:12px;border-bottom:#e5e3da 1px solid;' id='treeLink levlet_1' >");
									jw.print("&nbsp;&nbsp;<img src='image/jt1.gif'  width='5px'  height='10px' />&nbsp;<input type='checkbox' name='curruserid' style='display: none;line-height:40px;margin-right:5px;margin-top:10px;' id='curruserid' value='"
	...
	<div class="c_right">
			 ...
			 <div class="row">
			     <div class="col-xs-1">
				  ...
				  <div class="col-xs-3" style="margin-left: -28px;" >
				     	<button  onclick="select_ser_client('ajaxclientbus.vs')" class="btn btn-sm btn-primary" type="button" style="padding: 4px 8px;"><i class="icon icon-search"></i></button>
				  </div> 
				  
				  
				  <div class="btn-group" style="float: right;margin-right: 30px;">
				          
				           <select id="orderv" name="orderv" style="font-size: 15px;" onchange="select_orderby_client('ajaxclientbus.vs')">
				             <!-- 默认排序 -->
				  ...
				  <div class="btn-group" style="float: right;margin-right: 30px;">
				       <button onclick="bindbus()"   type="button" class="btn btn-sm btn-primary"><ww:property value='#session.local.a00712' /></button>
				       <button onclick="onhref('tobus.vs')"  type="button" class="btn btn-sm btn-primary"  ><ww:property value='#session.local.a00014' /></button><!-- 返回-->
				  </div>	
				   ...
				   <div class="btn-group" style="float: right;margin-right: 230px;margin-top: -23px;">
			             ...
			             <ww:property value='#session.local.a00688'/>:<input type="text" id="deviceId" name="deviceId" style="height: 20px;width: 50px;" />
				  </div>
		...
		<div id="showsonlist"> 
				...
				<div class="nav">
					<ul class="pg">
		                <!-- 上一页-->
						<li><a href="javascript:pageto_client(<ww:property value='qian' />,'ajaxclientbus.vs');" class="pg_f"></a></li>
			            <!--页码显示上-->
						<ww:iterator id="q"  value="ps.qs">
			                  <li><a href="javascript:pageto_client(<ww:property value='#q'/>,'ajaxclientbus.vs');" ><ww:property value="#q"  /></a> </li>
			            </ww:iterator>
			            
			            <!--当前页码-->
			            <li><a href="javascript:pageto_client(<ww:property value='#request.id'  />,'ajaxclientbus.vs');" style="background-color:#FFA534;"><ww:property value="#request.id"  /></a> </li>
			            
			             <!--页码显示下-->
			            <ww:iterator id="h"  value="ps.hs">
			                  <li><a href="javascript:pageto_client(<ww:property value='#h'/>,'ajaxclientbus.vs')"><ww:property value="#h"  /></a></li>
			            </ww:iterator>
			            
		                <!--下一页-->
			            <li><a href="javascript:pageto_client(<ww:property value='hou' />,'ajaxclientbus.vs');" class="pg_e"></a></li>
					</ul>
				    <!--<div class="jump_div"><input type="text" value="<ww:property value='id'/>"  id="goid"   class="jump_input"/>/<ww:property value="totalpage"/><input onclick="gogo_client('ajaxclientbig.vs')" type="button" value="<ww:property value='#session.local.a00080' />" class="jump_button"/></div>-->
					...
					<div class="pg_count">
					     ...
					     <a href="javascript:setpagetype_client(10,'ajaxclientbus.vs');" <ww:if test='pagetype==10'>style="background-color:#FFA534;"</ww:if> >10</a>
					     <a href="javascript:setpagetype_client(20,'ajaxclientbus.vs');" <ww:if test='pagetype==20'>style="background-color:#FFA534;"</ww:if> >20</a>
					     <a href="javascript:setpagetype_client(30,'ajaxclientbus.vs');" <ww:if test='pagetype==30'>style="background-color:#FFA534;"</ww:if> >30</a>
					     <a href="javascript:setpagetype_client(<ww:property value='total'/>,'ajaxclientbus.vs');" <ww:if test='pagetype==total'>style="background-color:#FFA534;"</ww:if> ><ww:property value='#session.local.a00815' /></a>
					</div>
