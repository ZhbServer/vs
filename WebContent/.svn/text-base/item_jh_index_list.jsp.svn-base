...
function jhiteminfo(jhid){
    window.location.href="jhiteminfo.vs?jhid="+jhid;   
}
//发布计划
function open_send_jh(itemc,jhid){
    if(itemc==0){
    ...
    layer.open({
        ...
        fix: false, //不固定
        shift: Math.ceil(Math.random()*6), //0-6的动画形式，-1不开启
        content:'tosendjh.vs?jhid='+jhid+"&jhtype=3",
        scrollbar: false,
        end: function(){ 
...
<div class="title bottom">
		<div class="t_container">
			<div class="loc">
			       <ww:property value='#session.local.a00065' />:<span class="function"><ww:property value='#session.local.a00645' /></span><span class="material_bg"><ww:property value='#session.local.a00646' /></span>
			</div>
		</div>
</div>
...
<!--				<li><a style="background-color:#B5B5B5;" href="tosclist.vs?sucai.type=1&gssi.sucaigroupid=0&pagetype=12&showtype=2"><ww:property value='#session.local.a00070' /></a></li>-->
		    ...
		    <div class="row">
		         ...
		         <div class="btn-group" style="float: right;margin-right: 50px;">
					  ...
					  <button onclick="open_add_jh(1,0)" type="button" class="btn btn-sm btn-primary" style="margin-left: 10px;"><ww:property value='#session.local.a00021' /></button><!-- 新增-->
			          <button onclick="del_jhs()"  type="button" class="btn btn-sm btn-danger"><ww:property value='#session.local.a00025' /></button><!-- 删除 -->			      
            ...
            <div style="height: 400px;margin-top:-5px;margin-left: 30px;">
               <table border="0" cellpadding="0" cellspacing="0" style="margin-top:0px;margin-bottom:0px;">
					...
					<tr class="cell_element cell_show">
					  ...
					  <td width="40%"><span style="cursor: pointer;" onclick="jhiteminfo(<ww:property value='id' />)"><ww:property value="name"/></span></td>
