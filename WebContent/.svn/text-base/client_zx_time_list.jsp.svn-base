...
function selzxtime(){
    ...
    document.form1.action = "zxtimeist.vs"; 
    document.form1.submit();
}

//节目详情
function zxtime_info(name,mark,ip){
	 layer.open({
		type: 2,
		area: ['1100px', '600px'],
        offset: '10px',
        ...
        content: "seeclientcountinfo.vs?name="+name+"&mark="+mark+"&ip="+ip+"&btime="+bt+"&etime="+et,
        scrollbar: false,
        end: function(){ 
...
function outzxtime(){
	...
	//document.form1.action = "outzxtime.vs";
	//document.form1.submit();
    ...
    layer.open({
        ...
        fix: false, //不固定
        shift: Math.ceil(Math.random()*6), //0-6的动画形式，-1不开启
        content: 'outzxtime.vs?sel='+sel+"&ser="+ser+"&datemode="+datemode+"&btime="+btime+"&etime="+etime,
        scrollbar: false,
        end: function(){ 
...
<div class="title bottom">
	<div class="t_container">
		<div class="loc">
	       ...
	       <span class="material_bg"><ww:property value='#session.local.a00856' /></span>
		</div>
	</div>
</div>
	...
	<div>
		<div class="c_right_header" >
	         <div style="float:left;margin: 10px 0px 10px 20px;width: 1030px;">
				  ...
				  <div>
				     	<button onclick="outzxtime()"  type="button" class="btn btn-sm btn-primary"><ww:property value='#session.local.a00658' /></button><!-- 导出日志-->
				  </div> 
		       </div>	
		</div>
          ...
          <div style="width:1030px;padding-right:17px;">
	            <table border="0" cellspacing="0" cellpadding="0">
	              <tr class="cell_element cell_first">
					...
					<td style="width:20%"><ww:property value='#session.local.a00856' /></td>
				...
				<table border="0" cellpadding="0" cellspacing="0">
					<ww:iterator value="cctList">
						...
						<tr class="cell_element cell_show">
						  ...
						  <td style="width:20%"><ww:property value='longtime' /></td>
					  	  <ww:if test="timediffer > 0">
					  	  	<td style="width:20%">
							  	<div style="text-align:center;">
							  		<span title="<ww:property value='#session.local.a00752' />" onclick="zxtime_info('<ww:property value='name'/>','<ww:property value='mark'/>','<ww:property value='ip'/>')" style="cursor: pointer;width:14px;height:14px;background-image: url('image/icon_all.png');background-position: -264px 0px;display: block;margin: 5px auto;"></span>
							  	</div>
						  	</td>
						  </ww:if>
						  <ww:else>
						  	<td style="width:10%">
								<ww:property value='date.substring(0,10)' />
							</td>
						  	<td style="width:10%">
						  		<span title="<ww:property value='#session.local.a00752' />" onclick="today_rcode('<ww:property value='id'/>')" style="cursor: pointer;width:14px;height:14px;background-image: url('image/icon_all.png');background-position: -264px 0px;display: block;margin: 5px auto;"></span>
							</td>
						  </ww:else>
						</tr>
					</ww:iterator>
		...
		<div class="nav" style="width: 1030px;">
			<ul class="pg">
                <!-- 上一页-->
				<li><a href="javascript:pageto(<ww:property value='qian' />,'zxtimeist.vs');" class="pg_f"></a></li>
	            <!--页码显示上-->
				<ww:iterator id="q"  value="ps.qs">
	                  <li><a href="javascript:pageto(<ww:property value='#q'/>,'zxtimeist.vs');" ><ww:property value="#q"  /></a> </li>
	            </ww:iterator>
	            
	            <!--当前页码-->
	            <li><a href="javascript:pageto(<ww:property value='#request.id'  />,'zxtimeist.vs');" style="background-color:#FFA534;"><ww:property value="#request.id"  /></a> </li>
	            
	             <!--页码显示下-->
	            <ww:iterator id="h"  value="ps.hs">
	                  <li><a href="javascript:pageto(<ww:property value='#h'/>,'zxtimeist.vs')"><ww:property value="#h"  /></a></li>
	            </ww:iterator>
	            
                <!--下一页-->
	            <li><a href="javascript:pageto(<ww:property value='hou' />,'zxtimeist.vs');" class="pg_e"></a></li>
			</ul>
			...
			<div class="pg_count">
			     ...
			     <a href="javascript:setpagetype(10,'zxtimeist.vs');" <ww:if test='#session.publicPagetype==10'>style="background-color:#FFA534;"</ww:if>  >10</a>
			     <a href="javascript:setpagetype(20,'zxtimeist.vs');" <ww:if test='#session.publicPagetype==20'>style="background-color:#FFA534;"</ww:if> >20</a>
			     <a href="javascript:setpagetype(30,'zxtimeist.vs');" <ww:if test='#session.publicPagetype==30'>style="background-color:#FFA534;"</ww:if> >30</a>
			     <a href="javascript:setpagetype(-1,'zxtimeist.vs');" <ww:if test='#session.publicPagetype==-1'>style="background-color:#FFA534;"</ww:if> ><ww:property value='#session.local.a00815' /></a>
