...
<!--			    <input type="button"  onclick="unBj()" value="<ww:property value='#session.local.a00272' />" style="width:55px; height:20px; text-align:center; background-color:#FCA41F; color:#ffffff; cursor:pointer;border: 0px;"/>-->
		...
		<div class=" div_title div_type">      
		   <!-- 
            ...
            <div>
            <!-- 有效期 -->
            <ww:property value="#session.local.a00641" />:
			<ww:property value="#session.local.a00540" />:
		    <input id="validitySdate" type="text" name="validitySdate"  style="height: 22px;width: 180px;"  value="<ww:property  value='validitySdate'/>"   onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
		    <!-- 开始时间 -->
		    <ww:property value="#session.local.a00541"/>:
            <input id="validityEdate" type="text" name="validityEdate"  style="height: 22px;width: 180px;"  value="<ww:property  value='validityEdate'/>"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
            <!-- 结束时间 -->
			<!-- 添加按钮 -->
		    <input type="button" onclick="savaSubmit()" value="<ww:property value='#session.local.a00080' />" style="width:50px; height:20px; text-align:center; background-color:#FCA41F; color:#ffffff; cursor:pointer;border: 0px;"/>
		    </div>
		</div>
		 <!-- 分组 -->
