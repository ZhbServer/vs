...
/*发送节目*/
 function send_dp_send(){
        ...
        //间隔时间
        var jgtime= document.getElementById("jgtime").value;
        //播放次数 
        var playcount=document.getElementById("playcount").value;
         ...
         if(jgtime==""){
        	 alert("<ww:property value='#session.local.a00586' />");
        	 return;
         }
         if(playcount==""){
        	 alert("<ww:property value='#session.local.a00587' />");
        	 return;
         }

         if(isNaN(jgtime)||isNaN(playcount)){
        	 alert("<ww:property value='#session.local.a00588' />");
        	 return;
         }
    	document.form1.action = "senddpitem.vs"; 
...
<div class="title bottom">
		<div class="t_container">
			...
			<div class="material_show" style="font-size: 14px;color:#343F51;height: 30px;line-height: 30px; ">
			   ...
			   <!-- 间隔时间(分钟) -->
			   <ww:property value="#session.local.a00584"/>:
			   <input   name="jgtime" id="jgtime" type="text" style="height: 23px;width: 50px;" /> 
               <!--播放次数-->
               <ww:property value="#session.local.a00585"/>:
			   <input   name="playcount" id="playcount"  type="text" style="height: 23px;width: 50px;" />
			   <button onclick="send_dp_send()"  type="button" class="btn btn-sm btn-primary"><ww:property value='#session.local.a00421' /></button>
			</div>
