...
function addjhitem(){
	...
	layer.open({
        ...
        content: 'toadjhitem.vs?igr.itemgroupid=0&jhid='+jhid+"&jhtype=2",
        scrollbar: false,
        end: function(){ 
		   ...
		   if(window.returnValue==undefined||window.returnValue==""){
		       ...
		       if ("Chrome" == mb) {
			    	window.location.href="jhinsertitemlist.vs?jhid="+jhid;
			   }else{
				    window.open("jhinsertitemlist.vs?jhid="+jhid,"_self");
			   }  
		   }               
		}
	});
...
function deljhitem(){
    ...
    layer.confirm("<ww:property value='#session.local.a00092' />?", {icon: 3, title:"<ww:property value='#session.local.a00093' />",btn: ["<ww:property value='#session.local.a00080' />", "<ww:property value='#session.local.a00014' />"]}, function(index,layero){
	    document.form1.action="deljhitem.vs?jhtype=2";
...
<!--				<li><a style="background-color:#B5B5B5;" href="tosclist.vs?sucai.type=1&gssi.sucaigroupid=0&pagetype=12&showtype=2"><ww:property value='#session.local.a00070' /></a></li>-->
			...
			<div class="row">
		         <div class="btn-group" style="float: right;margin-right: 50px;">
			         ...
			         <button onclick="onhref('jhlist.vs?jhtype=2')"      type="button" class="btn btn-sm btn-primary"><ww:property value='#session.local.a00655' /></button><!-- 返回 -->
         ...
         <div style="height: 400px;width: 1050px;margin-top: -20px;" >
           ...
           <div style="padding-right:17px;margin-left: 30px;">
               <table border="0" cellpadding="0" cellspacing="0" style="margin-bottom: 0px;">
					...
					<td width="20%"><ww:property value='#session.local.a00581' /></td><!-- 下载时间-->
		...
		<div class="nav" style="width: 1000px;margin-left:20px;">
			<ul class="pg">
                <!-- 上一页-->
				<li><a href="javascript:pageto(<ww:property value='qian' />,'jhinsertitemlist.vs');" class="pg_f"></a></li>
	            <!--页码显示上-->
				<ww:iterator id="q"  value="ps.qs">
	                  <li><a href="javascript:pageto(<ww:property value='#q'/>,'jhinsertitemlist.vs');" ><ww:property value="#q"  /></a> </li>
	            </ww:iterator>
	            
	            <!--当前页码-->
	            <li><a href="javascript:pageto(<ww:property value='#request.id'  />,'jhinsertitemlist.vs');" style="background-color:#FFA534;"><ww:property value="#request.id"  /></a> </li>
	            
	             <!--页码显示下-->
	            <ww:iterator id="h"  value="ps.hs">
	                  <li><a href="javascript:pageto(<ww:property value='#h'/>,'jhinsertitemlist.vs')"><ww:property value="#h"  /></a></li>
	            </ww:iterator>
	            
                <!--下一页-->
	            <li><a href="javascript:pageto(<ww:property value='hou' />,'jhinsertitemlist.vs');" class="pg_e"></a></li>
			</ul>
		    <!--<div class="jump_div"><input type="text" value="<ww:property value='id'/>"  id="goid"   class="jump_input"/>/<ww:property value="totalpage"/><input onclick="gogo('jhinsertitemlist.vs')" type="button" value="<ww:property value='#session.local.a00080' />" class="jump_button"/></div>-->
			...
			<div class="pg_count">
			     ...
			     <a href="javascript:setpagetype(10,'jhinsertitemlist.vs');" <ww:if test='pagetype==10'>style="background-color:#FFA534;"</ww:if>  >10</a>
			     <a href="javascript:setpagetype(20,'jhinsertitemlist.vs');" <ww:if test='pagetype==20'>style="background-color:#FFA534;"</ww:if> >20</a>
			     <a href="javascript:setpagetype(30,'jhinsertitemlist.vs');" <ww:if test='pagetype==30'>style="background-color:#FFA534;"</ww:if> >30</a>
			     <a href="javascript:setpagetype(<ww:property value='total'/>,'jhinsertitemlist.vs');" <ww:if test='pagetype==total'>style="background-color:#FFA534;"</ww:if> ><ww:property value='#session.local.a00815' /></a>
			</div>
